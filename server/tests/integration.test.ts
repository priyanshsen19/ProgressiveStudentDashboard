...
describe("Integration: happy path", () => {
  ...
  it("login -> dashboard -> complete lesson -> dashboard reflects the change", async () => {
    ...
    const before = await request(app).get("/dashboard").set(auth).expect(200);
    ...
    const after = await request(app).get("/dashboard").set(auth).expect(200);
